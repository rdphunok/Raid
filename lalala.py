...
MASTER_TOKEN = "8666502260:AAG52YosLmCHkQvdY5_oSlpItsBy_zoiyjM"  # Primary Master Gateway Token
OWNER_ID = 7699820685                        # Root Administrator User ID

# Valid Direct Image URL for Menu Banner
MENU_IMAGE_URL = "https://files.catbox.moe/k3b7g5.jpg"  

# 5 Worker Nodes Pool (Background Silent Execution Engines)
SWIPE_BOT_TOKENS = [
...
# Optimized dispatch with timeout handling to prevent ReadTimeout/TimedOut errors
...
# --- MASTER ROUTER FOR CUSTOM EXCLAMATION PREFIX (!) ---
...
async def process_incoming_surveillance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message or not update.main_chat and not update.message.from_user: return
...
def main():
    # Configure custom HTTPX request with higher timeouts for the master bot as well
    custom_req = HTTPXRequest(connect_timeout=30.0, read_timeout=30.0)
    ...
    logging.info("SIREN v3.8 Custom Prefix Framework initialized successfully.")
    app.run_polling()
