...
async def process_custom_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text.startswith("!"):
        # Pass to surveillance pipeline if not a command
    ...
    # 8. Swipe Modules (!sw, !se, !sh, !cw) & Stops
    elif cmd == "sw":
        if not verify_operator_authorization(operator_id) or not update.message.reply_to_message: return
    ...
    # 9. Auto-Delete Lockdown (!shh / !bol)
    elif cmd == "shh":
        if not verify_operator_authorization(operator_id) or not update.message.reply_to_message: return
...
def main():
    ...
    # Single message handler to process custom prefix commands starting with '!'
