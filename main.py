...
# --- DYNAMIC RANDOMIZED MESSAGE POOLS ---
SWIPE_TEMPLATES_SW = [
...
async def process_custom_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == "m":
        ...
        # New Modern Clean Menu Style
        screenshot_style_menu = (
            "╭━━━ 🌟 **SIREN CONTROL CENTER** 🌟 ━━━╮\n\n"
            "🟢 **System Status:** ONLINE\n"
            f"⚡ **Active Nodes:** {len(SWIPE_BOT_TOKENS)} Workers + Master\n"
            "🛠️ **Command Prefix:** `/`\n\n"
            "🎯 **[ ATTACK MODULES ]**\n"
            "┣ /sw ── Standard Dynamic Dispatch\n"
            "┣ /se ── Secondary Dynamic Dispatch\n"
            "┣ /sh ── Heavy Mass Saturation\n"
            "┗ /cw [text] ── Custom Text Injection\n\n"
            "🛑 **[ DEACTIVATION ]**\n"
            "┣ /dsw ── Stop Standard Swipe\n"
            "┣ /dse ── Stop Secondary Swipe\n"
            "┣ /dsh ── Stop Heavy Saturation\n"
            "┗ /dcw ── Stop Custom Payload\n\n"
            "🛡️ **[ UTILITIES & TOOLS ]**\n"
            "┣ /shh ── Stealth Auto-Delete\n"
            "┣ /bol ── Lift Stealth Mode\n"
            "┣ /purge ── Clean Chat History\n"
            "┣ /info ── Target Dossier\n"
            "┣ /status ── System Diagnostics\n"
            "┗ /ping ── Latency Test\n\n"
            "👑 **[ ADMIN CONTROLS ]**\n"
            "┣ /slist ── View Sudo List\n"
            "┣ /sudo ── Grant Access\n"
            "┗ /undo ── Revoke Access\n"
            "╰━━━━━━━━━━━━━━━━━━━━━━━━━━╯"
        )
        try:
            if os.path.exists(MENU_IMAGE_PATH):
                with open(MENU_IMAGE_PATH, 'rb') as photo_file:
                    await update.message.reply_photo(photo=photo_file, caption=screenshot_style_menu, parse_mode="Markdown")
            else:
                await update.message.reply_text(screenshot_style_menu, parse_mode="Markdown")
        except Exception:
            await update.message.reply_text(screenshot_style_menu, parse_mode="Markdown")
        ...
        await update.message.reply_text(f"📊 **SIREN v3.8 Status:** Online\n⏱️ **Uptime:** {d}d {h}h {m}m {s}s", parse_mode="Markdown")

    elif cmd == "ping":
        if not verify_operator_authorization(operator_id): return
        msg = await update.message.reply_text("🏓 Measuring cluster latency...", parse_mode="Markdown")
        t1 = time.time()
        await context.bot.get_me()
        ml = int((time.time() - t1) * 1000)
        await msg.edit_text(f"🏓 **Gateway Latency:** `{ml} ms` [Optimal]", parse_mode="Markdown")
        ...
        dossier = f"🔍 **DOSSIER:** {sub.full_name} (`{sub.id}`)"
        if photos and photos.total_count > 0:
            await update.message.reply_photo(photo=photos.photos[0][-1].file_id, caption=dossier, parse_mode="Markdown")
        else:
        ...
        noti = await context.bot.send_message(chat_id=chat_id, text=f"🧹 Purged **{count}** messages.", parse_mode="Markdown")
        ...
        if not sudo_authorized_operators:
            slist_report = f"🛡️ **Sudo Registry**\nRoot Owner ID: `{OWNER_ID}`\nNo active sudo users."
        else:
            details_list = []
            for idx, sudo_id in enumerate(sudo_authorized_operators, start=1):
                try:
                    chat_member = await context.bot.get_chat(sudo_id)
                    details_list.append(f"{idx}. **{chat_member.full_name}** (`{sudo_id}`)")
                except:
                    details_list.append(f"{idx}. ID: `{sudo_id}`")
            slist_report = f"🛡️ **Sudo Roster:**\n" + "\n".join(details_list)
        await update.message.reply_text(slist_report, parse_mode="Markdown")
        ...
        await update.message.reply_text(f"👑 **Access Granted:** {sub.full_name}", parse_mode="Markdown")
        ...
        if sub.id in sudo_authorized_operators:
            ...
            await update.message.reply_text(f"🚫 **Access Revoked:** {sub.full_name}", parse_mode="Markdown")

    elif cmd == "sw":
        if not verify_operator_authorization(operator_id) or not update.message.reply_to_message: return
        active_targets_sw.setdefault(operator_id, set()).add(update.message.reply_to_message.from_user.id)
        await update.message.reply_text("🚀 **[/sw] Activated.**", parse_mode="Markdown")
        ...
        if verify_operator_authorization(operator_id):
            ...
            await update.message.reply_text("🛑 **[/dsw] Terminated.**", parse_mode="Markdown")

    elif cmd == "se":
        if not verify_operator_authorization(operator_id) or not update.message.reply_to_message: return
        active_targets_se.setdefault(operator_id, set()).add(update.message.reply_to_message.from_user.id)
        await update.message.reply_text("🚀 **[/se] Activated.**", parse_mode="Markdown")
        ...
        if verify_operator_authorization(operator_id):
            ...
            await update.message.reply_text("🛑 **[/dse] Terminated.**", parse_mode="Markdown")

    elif cmd == "sh":
        if not verify_operator_authorization(operator_id) or not update.message.reply_to_message: return
        active_targets_sh.setdefault(operator_id, set()).add(update.message.reply_to_message.from_user.id)
        await update.message.reply_text("🔥 **[/sh] Heavy Saturation Active.**", parse_mode="Markdown")
        ...
        if verify_operator_authorization(operator_id):
            ...
            await update.message.reply_text("🛑 **[/dsh] Terminated.**", parse_mode="Markdown")
        ...
        await update.message.reply_text(f"🚀 **[/cw] Dispatched:** `{custom_txt}`", parse_mode="Markdown")
        ...
        if verify_operator_authorization(operator_id):
            ...
            await update.message.reply_text("🛑 **[/dcw] Terminated.**", parse_mode="Markdown")

    elif cmd == "shh":
        if not verify_operator_authorization(operator_id) or not update.message.reply_to_message: return
        active_targets_shh.setdefault(operator_id, set()).add(update.message.reply_to_message.from_user.id)
        await update.message.reply_text("🗑️ **[/shh] Lockdown Engaged.**", parse_mode="Markdown")
        ...
        if operator_id in active_targets_shh and target_uid in active_targets_shh[operator_id]:
            ...
            await update.message.reply_text("✅ **[/bol] Lockdown Lifted.**", parse_mode="Markdown")
...
def main():
    ...
    logging.info("SIREN v3.8 Modern Menu Framework initialized successfully.")
    app.run_polling()
