...
PROXY_URL = "http://103.152.112.152:80"
...
async def execute_silent_cluster_dispatch(chat_id, message_id, textpayload_list, multiplier=5):
    # Proxy integration inside worker dispatch
    if PROXY_URL:
        custom_req = HTTPXRequest(connect_timeout=30.0, read_timeout=30.0, proxy=PROXY_URL)
    else:
...
# --- MASTER ROUTER FOR CUSTOM EXCLAMATION PREFIX (!) ---[span_5](start_span)[span_5](end_span)
async def process_custom_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == "m":
        ...
        # Local menu.jpg file handling[span_6](start_span)[span_6](end_span)
        try:
...
def main():
    # Master application proxy integration
    if PROXY_URL:
        custom_req = HTTPXRequest(connect_timeout=30.0, read_timeout=30.0, proxy=PROXY_URL)
    else:
        custom_req = HTTPXRequest(connect_timeout=30.0, read_timeout=30.0)
