...
MASTER_TOKEN = "8666502260:AAG52YosLmCHkQvdY5_oSlpItsBy_zoiyjM"  # Primary Master Gateway Token[span_3](start_span)[span_3](end_span)
OWNER_ID = 7699820685                        # Root Administrator User ID[span_4](start_span)[span_4](end_span)

# Local Image File Name (script ke sath folder mein menu.jpg honi chahiye)[span_5](start_span)[span_5](end_span)
MENU_IMAGE_PATH = "menu.jpg"  

# 5 Worker Nodes Pool (Background Silent Execution Engines)[span_6](start_span)[span_6](end_span)
SWIPE_BOT_TOKENS = [
...
# --- DYNAMIC RANDOMIZED MESSAGE POOLS ---[span_7](start_span)[span_7](end_span)
SWIPE_TEMPLATES_SW = [
...
async def process_custom_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == "m":
        ...
        screenshot_style_menu = (
            ...
            "<code>/inf</code> » Advanced Target Dossier\n"
        ...
        # Determine target user (reply target or sender)
        ...
        # Fetch Profile Photos and Chat Member details
        photos = await context.bot.get_user_profile_photos(user_id=target_user.id, limit=1)
        
        chat_member_status = "Unknown / Private"
        try:
            if chat_id != update.effective_user.id:
                member_info = await context.bot.get_chat_member(chat_id=chat_id, user_id=target_user.id)
                chat_member_status = member_info.status.capitalize()
        except Exception:
            pass

        username_str = f"@{target_user.username}" if target_user.username else "No Username"
        is_bot_str = "Yes 🤖" if target_user.is_bot else "No 👤"
        lang_str = target_user.language_code.upper() if target_user.language_code else "Not Specified"
        premium_str = "Yes ✨" if getattr(target_user, "is_premium", False) else "No"
        photo_count = photos.total_count if photos else 0

        dossier = (
            f"⚡ <b>ADVANCED INTELLIGENCE DOSSIER</b> ⚡\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"👤 <b>Full Name:</b> {target_user.full_name}\n"
            f"🆔 <b>User ID:</b> <code>{target_user.id}</code>\n"
            f"🔗 <b>Username:</b> {username_str}\n"
            f"🌐 <b>Language Code:</b> {lang_str}\n"
            f"💎 <b>Telegram Premium:</b> {premium_str}\n"
            f"🤖 <b>Is Bot Account:</b> {is_bot_str}\n"
            f"🛡️ <b>Chat Status / Role:</b> {chat_member_status}\n"
            f"🖼️ <b>Profile Photos Found:</b> {photo_count}\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"📌 <i>Requested by:</i> {update.effective_user.full_name}"
        )
        ...
        await update.message.reply_text("🗑️ <b>[/shh] Lockdown Engaged (Text & Media).</b>", parse_mode="HTML")
...
def main():
    ...
    app.add_handler(MessageHandler(filters.ALL & ~filters.COMMAND, process_custom_commands))
