...
# ==========================================
# COMPLETELY SAFE COMMAND FILTER (v21+ Compatible)
# ==========================================

def make_command_filter(command_name):
    async def filter_func(update: Update) -> bool:
        if not update.message or not update.message.text:
            return False
        text = update.message.text.strip()
        expected = f"{CURRENT_PREFIX}{command_name}"
        return text == expected or text.startswith(expected + " ")
    return filters.TEXT & filters.UpdateType.MESSAGE & filters.Create(filter_func)
...
async def run_bot(token, is_master):
    ...
    # --- REACTION COMMANDS REGISTERED FOR ALL BOTS (Master + Secondary) ---
    app.add_handler(MessageHandler(make_command_filter("rct"), start_reaction))
    app.add_handler(MessageHandler(make_command_filter("drct"), stop_reaction))
    app.add_handler(MessageHandler(make_command_filter("arct"), start_global_reaction))
    app.add_handler(MessageHandler(make_command_filter("darct"), stop_global_reaction))

    # --- ALL OTHER COMMANDS REGISTERED ONLY FOR MASTER BOT ---
    if is_master:
        app.add_handler(CommandHandler("status", status_command))
        app.add_handler(MessageHandler(make_command_filter("status"), status_command))
        
        app.add_handler(CommandHandler("menu", menu_command))
        app.add_handler(MessageHandler(make_command_filter("menu"), menu_command))
        
        app.add_handler(MessageHandler(make_command_filter("pre"), set_prefix))
        app.add_handler(MessageHandler(make_command_filter("ptst"), ping_test_command))
        app.add_handler(MessageHandler(make_command_filter("tag"), tag_all_command))
        app.add_handler(MessageHandler(make_command_filter("clall"), start_clear_all))
        app.add_handler(MessageHandler(make_command_filter("dclall"), stop_clear_all))
        app.add_handler(MessageHandler(make_command_filter("whois"), whois_command))
        app.add_handler(MessageHandler(make_command_filter("tts"), tts_command))
        app.add_handler(MessageHandler(make_command_filter("sudo"), add_sudo))
        app.add_handler(MessageHandler(make_command_filter("unsudo"), remove_sudo))
        
        app.add_handler(CommandHandler("sudos", list_sudo))
        app.add_handler(MessageHandler(make_command_filter("sudos"), list_sudo))
        
        app.add_handler(MessageHandler(make_command_filter("raid"), start_raid))
        app.add_handler(MessageHandler(make_command_filter("draid"), stop_raid))
        app.add_handler(CommandHandler("re", start_reply))
        app.add_handler(MessageHandler(make_command_filter("re"), start_reply))
        app.add_handler(CommandHandler("dre", stop_reply))
        app.add_handler(MessageHandler(make_command_filter("dre"), stop_reply))
        app.add_handler(MessageHandler(make_command_filter("cre"), start_custom_reply))
        app.add_handler(MessageHandler(make_command_filter("dcre"), stop_custom_reply))
