...
# ==========================================
# STANDARD COMMAND REGISTRATION HANDLER
# ==========================================

def add_cmd(app, command_name, callback_func, is_all_bots=False):
    # Standard CommandHandler handles prefixes and commands natively
    app.add_handler(CommandHandler(command_name, callback_func))
...
async def run_bot(token, is_master):
    ...
    # --- REACTION COMMANDS (Registered for All Bots) ---
    for cmd in ["rct", "drct", "arct", "darct"]:
        app.add_handler(CommandHandler(cmd, start_reaction if cmd=="rct" else (stop_reaction if cmd=="drct" else (start_global_reaction if cmd=="arct" else stop_global_reaction))))
