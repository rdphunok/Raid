...
async def menu_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    professional_caption = (
        ...
        f" └── 👑 **Owner:** `{OWNER_HANDLE}`\n\n"
        f" 🌩️ **─── [ RAID & BROADCAST ] ───**\n"
        f"  ├─ `/tag <text>` ➔ Mass notify\n"
        f"  ├─ `/raid <tgt>` ➔ Thread flood\n"
        f"  └─ `/draid` ➔ Stop/Halt raid\n\n"
        f" 🛡️ **─── [ SECURITY & ADMIN ] ───**\n"
        f"  ├─ `/clall` ➔ Lockdown chat\n"
        f"  ├─ `/dclall` ➔ Lift lockdown\n"
        f"  ├─ `/sudo` ➔ Authorize node\n"
        f"  └─ `/unsudo` ➔ Revoke node\n\n"
        f" 🎯 **─── [ REACTIONS & SPAM ] ───**\n"
        f"  ├─ `/arct <emoji>` ➔ Global auto-react\n"
        f"  ├─ `/darct` ➔ Stop global react\n"
        f"  ├─ `/rct <emoji>` ➔ Target auto-react\n"
        f"  ├─ `/drct` ➔ Stop target react\n"
        f"  ├─ `/re` ➔ Start template loop\n"
        f"  ├─ `/dre` ➔ Stop template loop\n"
        f"  ├─ `/cre <txt>` ➔ Start custom loop\n"
        f"  └─ `/dcre` ➔ Stop custom loop\n\n"
        f" 🛠️ **─── [ SYSTEM & UTILS ] ───**\n"
        f"  ├─ `/ptst` ➔ Latency ping\n"
        f"  ├─ `/whois` ➔ Target intel\n"
        f"  ├─ `/tts <txt>` ➔ Voice synthesis\n"
        f"  ├─ `/status` ➔ Core telemetry\n"
        f"  └─ `/sudos` ➔ Admin registry\n\n"
        f"└───────────────────────────┘\n"
...
async def run_bot(token, is_master):
    ...
    app.add_handler(CommandHandler("rct", start_reaction))
    app.add_handler(CommandHandler("drct", stop_reaction))
    app.add_handler(CommandHandler("arct", start_global_reaction))
    app.add_handler(CommandHandler("darct", stop_global_reaction))
    ...
    if is_master:
        ...
        app.add_handler(CommandHandler("ptst", ping_test_command))
        app.add_handler(CommandHandler("tag", tag_all_command))
        app.add_handler(CommandHandler("clall", start_clear_all))
        app.add_handler(CommandHandler("dclall", stop_clear_all))
        app.add_handler(CommandHandler("whois", whois_command))
        app.add_handler(CommandHandler("tts", tts_command))
        app.add_handler(CommandHandler("sudo", add_sudo))
        app.add_handler(CommandHandler("unsudo", remove_sudo))
        app.add_handler(CommandHandler("sudos", list_sudo))
        app.add_handler(CommandHandler("raid", start_raid))
        app.add_handler(CommandHandler("draid", stop_raid))
        app.add_handler(CommandHandler("re", start_reply))
        app.add_handler(CommandHandler("dre", stop_reply))
        app.add_handler(CommandHandler("cre", start_custom_reply))
        app.add_handler(CommandHandler("dcre", stop_custom_reply))
