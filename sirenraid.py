...
async def menu_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    professional_caption = (
        ...
        f"└───────────────────────────┘\n"
        f" ├── 🌐 **Status:** `ONLINE [SECURE]`[span_0](start_span)[span_0](end_span)\n"
        f" ├── 👑 **Owner:** `{OWNER_HANDLE}`[span_1](start_span)[span_1](end_span)\n"
        f" └── ⚙️ **Prefix:** `[ {p} ]`[span_2](start_span)[span_2](end_span)\n\n"
        f" 🌩️ **─── [ RAID & BROADCAST ] ───**\n"
        f"  ├─ `{p}tag <text>` ➔ Mass notify[span_3](start_span)[span_3](end_span)\n"
        f"  ├─ `{p}raid <tgt>` ➔ Thread flood[span_4](start_span)[span_4](end_span)\n"
        f"  └─ `{p}draid` ➔ Stop/Halt raid[span_5](start_span)[span_5](end_span)\n\n"
        f" 🛡️ **─── [ SECURITY & ADMIN ] ───**\n"
        f"  ├─ `{p}clall` ➔ Lockdown chat[span_6](start_span)[span_6](end_span)\n"
        f"  ├─ `{p}dclall` ➔ Lift lockdown[span_7](start_span)[span_7](end_span)\n"
        f"  ├─ `{p}sudo` ➔ Authorize node[span_8](start_span)[span_8](end_span)\n"
        f"  └─ `{p}unsudo` ➔ Revoke node[span_9](start_span)[span_9](end_span)\n\n"
        f" 🎯 **─── [ REACTIONS & SPAM ] ───**\n"
        f"  ├─ `{p}arct <emoji>` ➔ Global auto-react[span_10](start_span)[span_10](end_span)\n"
        f"  ├─ `{p}darct` ➔ Stop global react[span_11](start_span)[span_11](end_span)\n"
        f"  ├─ `{p}rct <emoji>` ➔ Target auto-react[span_12](start_span)[span_12](end_span)\n"
        f"  ├─ `{p}drct` ➔ Stop target react[span_13](start_span)[span_13](end_span)\n"
        f"  ├─ `{p}re` ➔ Start template loop[span_14](start_span)[span_14](end_span)\n"
        f"  ├─ `{p}dre` ➔ Stop template loop[span_15](start_span)[span_15](end_span)\n"
        f"  ├─ `{p}cre <txt>` ➔ Start custom loop[span_16](start_span)[span_16](end_span)\n"
        f"  └─ `{p}dcre` ➔ Stop custom loop[span_17](start_span)[span_17](end_span)\n\n"
        f" 🛠️ **─── [ SYSTEM & UTILS ] ───**\n"
        f"  ├─ `{p}ptst` ➔ Latency ping[span_18](start_span)[span_18](end_span)\n"
        f"  ├─ `{p}pre <sym>` ➔ Change prefix[span_19](start_span)[span_19](end_span)\n"
        f"  ├─ `{p}whois` ➔ Target intel[span_20](start_span)[span_20](end_span)\n"
        f"  ├─ `{p}tts <txt>` ➔ Voice synthesis[span_21](start_span)[span_21](end_span)\n"
        f"  ├─ `{p}status` ➔ Core telemetry[span_22](start_span)[span_22](end_span)\n"
        f"  └─ `{p}sudos` ➔ Admin registry[span_23](start_span)[span_23](end_span)\n\n"
        f"└───────────────────────────┘\n"
        f"  🔒 *Engine Managed by {OWNER_HANDLE}*[span_24](start_span)[span_24](end_span)"
    )
...
async def stop_raid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in active_raid_chats:
        ...
        await update.message.reply_text("⚠️ **State Notice:** No active raid sequence found in this chat context.")
